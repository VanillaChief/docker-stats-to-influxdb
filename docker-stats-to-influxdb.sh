...
####################
### Use LANG=en_US.UTF-8 to /etc/default/locale
...
while :
    ...
    # printf "%s\n" "${docker_stats[@]}" | awk '{print -e $3"\t"$7"\t"$4"/"$6"\t"$8"/"$10"\t"$11"/"$13"\t"$2}' && echo
    printf "%s\n" "${docker_stats[@]}" | while read line
        ...
        echo -e "$cpu\t$mem_use_proc\t$mem_use/$mem_all\t$net_in/$net_out\t$disk_in/$disk_out\t$container"
        ### Output to InfluxDB
        data="$table,host=$host,container=$container cpu_use%=$cpu,mem_use%=$mem_use_proc,mem_use=$mem_use,mem_all=$mem_all,"
        data+="net_in=$net_in,net_out=$net_out,disk_in=$disk_in,disk_out=$disk_out $date"
        ...
        if [ $num -ne 1 ]
            ...
            echo -e "$cpu_sum\t$mem_use_proc_sum\t$mem_use_sum/$mem_all\t$net_in_sum/$net_out_sum\t$disk_in_sum/$disk_out_sum\tSUM"
            data="$table,host=$host,container=sum cpu_use%=$cpu_sum,mem_use%=$mem_use_proc_sum,mem_use=$mem_use_sum,mem_all=$mem_all,"
            data+="net_in=$net_in_sum,net_out=$net_out_sum,disk_in=$disk_in_sum,disk_out=$disk_out_sum $date"
            curl -s -o /dev/null -i -XPOST "http://$ip:$port/write?db=$db" --data-binary "$data"
        fi
    done
    sleep 5
